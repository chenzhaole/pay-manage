...
//        }
    ...
    @Override
	public boolean isExisted(Map<String, Object> eqMap, Map<String, Object> neMap) {
        DetachedCriteria dc = createDetachedCriteria();
        for (String key : neMap.keySet()) {
            dc.add(Restrictions.ne(key, neMap.get(key)));
        }
        Criterion or = null;
        for (String key : eqMap.keySet()) {
            if (or == null) {
                or = Restrictions.eq(key, eqMap.get(key));
            } else {
                or = Restrictions.or(or, Restrictions.eq(key, eqMap.get(key)));
            }
        }
        if (or != null) {
            dc.add(or);
