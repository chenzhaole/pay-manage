...
function deleteProduct(obj) {
    ...
    var nowTr = $(obj).parent().parent().parent().parent();
    $(nowTr).insertBefore($(nowTr).prev());
    for (var i = 0; i < $("#productTable tr").length; i++) {
        $("#productTable tr").eq(i).find("td:first").find("span").text("顺序 " + (i + 1));
        $("#productTable tr").eq(i).find("input[type='hidden']").val((i + 1));
    }
}
