...
<body>
	...
	<form:form id="searchForm" action="${ctx}/sys/log/" method="post" class="breadcrumb form-search">
		...
		<div>
			<label>用户 ID：</label><input id="createById" name="createById" type="text" maxlength="50" class="input-medium" value="${createById}"/>
			<label>操作URI：</label><input id="requestUri" name="requestUri" type="text" maxlength="200" class="input-xxlarge" value="${requestUri}"/>
        ...
        <div style="margin-top:8px;">
			<label>日期范围：&nbsp;</label><input id="beginDate" name="beginDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
				value="${beginDate}" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true,isShowOK:false,isShowToday:false});"/>
			<label>&nbsp;--&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</label><input id="endDate" name="endDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
				value="${endDate}" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:true,isShowOK:false,isShowToday:false});"/>&nbsp;&nbsp;
			&nbsp;&nbsp;&nbsp;<input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/>&nbsp;&nbsp;
		</div>
	</form:form>
	<tags:message content="${message}"/>
	<table id="contentTable" class="table table-striped table-bordered table-condensed" width="100%">
		<thead><tr><th>所在机构</th><th>所在部门</th><th>操作用户</th><th>URI</th><th>提交方式</th><th>操作者IP</th><th>创建时间</th></tr></thead>
		<tbody>
		<c:forEach items="${page.list}" var="log">
			...
			<tr>
				<td colspan="8" style="white-space: normal;word-break: break-all;word-wrap: break-word;">用户代理: ${log.userAgent}<br/>提交参数: ${fns:escapeHtml(log.params)}
