	           ...
	           <ul id="menu" class="nav">
				 ...
				 <c:forEach items="${fns:getMenuList()}" var="menu" varStatus="idxStatus">
					<c:if test="${menu.parent.id eq 1&&menu.isShow eq 1}">
						<li class="menu ${firstMenu?' active':''}"><a id="top_menu_${menu.id}" class="menu" href="${ctx}/sys/menu/tree?parentId=${menu.id}" target="menuFrame" >${menu.name}</a></li>
						<c:if test="${firstMenu}">
...
<!-- 			     </li> -->
				  ...
				  <%--  <c:if test="${fns:isCurrentPlatform() && fns:getPortalAddress() != null && fns:getPortalAddress() != ''}">
				   	<li><a href="${fns:getPortalAddress()}" title="进入门户" target="_blank">进入门户</a></li>
				   </c:if> --%>
