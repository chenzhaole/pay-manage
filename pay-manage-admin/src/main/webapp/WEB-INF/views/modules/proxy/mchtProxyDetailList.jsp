                ...
                <div class="control-group">
                    <div class="controls">
                        ...
                        <select name="payStatus" class="input-medium" id="payStatus">
                            ...
                            <option <c:if test="${paramMap.payStatus == '21'}">selected</c:if> value="21">代付成功</option>
                            <option <c:if test="${paramMap.payStatus == '22'}">selected</c:if> value="22">代付失败</option>
                            <option <c:if test="${paramMap.payStatus == '23'}">selected</c:if> value="23">代付处理中</option>
                            <option <c:if test="${paramMap.payStatus == '25'}">selected</c:if> value="25">未知,需人工确认</option>
                            <option <c:if test="${paramMap.payStatus == '30'}">selected</c:if> value="30">已提交</option>
                            <option <c:if test="${paramMap.payStatus == '31'}">selected</c:if> value="31">提交成功</option>
                            <option <c:if test="${paramMap.payStatus == '32'}">selected</c:if> value="32">提交失败</option>
                        </select>&nbsp;&nbsp;&nbsp;
                    </div>
                </div>
            </td>
