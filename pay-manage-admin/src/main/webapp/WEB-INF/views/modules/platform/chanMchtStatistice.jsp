		...
		<tbody>
				<c:forEach items="${list}" var="report">
					<tr>
						...
						<td>${report.mchtReqNum}</td>
						<td>${report.compInterceptNum }</td>
						<td>${report.chanCommitNum }</td>
						<td>${report.chanCommitSuccNum }</td>
						<td>${report.chanCommitFailNum }</td>
						<td>${report.paySuccNum}</td>
						<td>
							<c:choose>
							<c:when test="${report.chanCommitBackTimeoutNum ==null || report.chanCommitBackTimeoutNum==0}">
								100.00%
							</c:when>
							<c:otherwise>
