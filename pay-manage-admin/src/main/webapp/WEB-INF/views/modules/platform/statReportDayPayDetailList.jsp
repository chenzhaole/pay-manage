...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $("#btnExportChongzhi").click(function () {
                $("#searchFormChongzhi").attr("action", "${ctx}/platform/statReportDayPayDetail/exportChongzhi");
                $("#searchFormChongzhi").submit();
                $("#searchFormChongzhi").attr("action", "${ctx}/platform/statReportDayPayDetail/list");
            });
        });
    </script>
</head>
...
<form id="searchFormChongzhi" action="${ctx}/platform/statReportDayPayDetail/list" method="post" class="breadcrumb form-search">
    ...
    <table>
        <tr>
            <td colspan="2">
                <div class="btn-group">
                    <label class="control-label">充值业务  </label>&nbsp;&nbsp;<input id="btnExportChongzhi" class="btn btn-primary" type="button" value="导出"/>
                </div>
            </td>
        </tr>
    </table>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed table-hover"
    ...
    <tbody>
    <c:forEach items="${chongzhiList}" var="info">
        <tr>
            <td>${info.tradeDate}</td>
            <td>${info.agentMchtName}</td>
            <td>${info.payMchtName}</td>
