...
<div class="aui-content aui-padded-15 ">
    <div class="qr_img_box">
        ...
        <p class="qr_box">
            <img class="qr_img" src="${result.imgUrl}" alt="" id="code"/>
        </p>

        <p class="pay_tip1">步骤1&nbsp;:&nbsp;点击截屏或者长按二维码,保存至相册</p>

        <c:choose>
            <c:when test="${supportPayment eq 'unionpay'}">
            ...
            <c:otherwise>
                <p class="pay_tip2">步骤2&nbsp;:&nbsp;前往
                    <span>
								<c:if test="${supportPayment eq 'wxpay'}">微信</c:if>
								<c:if test="${supportPayment eq 'alipay'}">支付宝</c:if>
								<c:if test="${supportPayment eq 'qqpay'}">QQ</c:if>
