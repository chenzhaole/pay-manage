...
public class GwCashierPlatController extends GwCashierBaseController {
     ...
     * 解析并校验请求参数
     */
    @RequestMapping(value="/platCall/{mchtId}/{mchtOrderId}/{paymentType}/{extraData}")
    public String platCall(HttpServletRequest request, Model model, @PathVariable String mchtId, @PathVariable String mchtOrderId, @PathVariable String paymentType, @PathVariable String extraData){
        ...
        String mobile = "";
        try {
            midoid = "商户号：" + mchtId +"，商户订单号：" + mchtOrderId + ",随机数extraData：" + extraData + "-->支付类型：" + paymentType + "-->";
