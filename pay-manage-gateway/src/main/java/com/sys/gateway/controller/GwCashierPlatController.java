...
public class GwCashierPlatController extends GwCashierBaseController {
    ...
    private static final String BIZ_DESC = "网页支付_收银台发起请求-> ";
    ...
    public String platPcCall(HttpServletRequest request, @PathVariable String mchtId, @PathVariable String mchtOrderId, @PathVariable String paymentType, @PathVariable String extraData) {
        ...
        String moid = "";
        try {
            moid = "pc端收银台异步下单，商户号：" + mchtId + "，商户订单号：" + mchtOrderId + ",随机数extraData：" + extraData + "-->支付类型：" + paymentType + "-->";
            String bankCode = request.getParameter("bankCode");
            //通过程序判断，由于不同的设备类型对应页面不一样，且掉支付的方式也不一样，所以会根据设备类型来做判断
            logger.info(BIZ_DESC + moid + "通过程序获取deviceType的值");
            //根据userAgent判断设备类型：pc、手机端、微信内(针对公众号支付)
            String userAgent = getUserAgentInfoByRequest(request, moid, BIZ_DESC);
            logger.info(BIZ_DESC + moid + "根据请求头获取的user-agent为：" + userAgent);
            deviceType = HttpUtil.getDeviceType(userAgent);
            logger.info(BIZ_DESC + moid + "通过程序判断deviceType为：" + deviceType + "-->【1：手机端，2：pc端，3：微信内，4：支付宝内】");
            if (StringUtils.isNotBlank(mchtId) && StringUtils.isNotBlank(mchtOrderId) && StringUtils.isNotBlank(paymentType) && StringUtils.isNotBlank(extraData)) {
                ...
                logger.info(BIZ_DESC + moid + "通过程序获取的ip为：" + ip);
                //调用handler处理业务
                logger.info(BIZ_DESC + moid + "调用ITradeCashierPlatHandler处理业务逻辑，传入的请求参数是mchtId=" + mchtId + ", mchtOrderId=" + mchtOrderId + ", paymentType=" + paymentType + ", extraData=" + extraData + ", deviceType=" + deviceType + ", ip=" + ip);
                result = iTradeCashierPlatHandler.process(mchtId, mchtOrderId, paymentType, extraData, deviceType, ip, bankCode);
                logger.info(BIZ_DESC + moid + "调用ITradeCashierPlatHandler处理业务逻辑，传入的请求参数是mchtId=" + mchtId + ", mchtOrderId=" + mchtOrderId + ", paymentType=" + paymentType + ", extraData=" + extraData + ", deviceType=" + deviceType + ", ip=" + ip + "，返回的数据为：" + JSONObject.toJSONString(result));
                if (null != result && ErrorCodeEnum.SUCCESS.getCode().equals(result.getRespCode()) && null != result.getData()) {
                    ...
                    if (isUseChanCashierPage(result.getData(), moid, BIZ_DESC)) {
                        //判断是否直接跳向上游收银台
                        pcAsynScanInfo = returnChanCasgierPageInfo(resultInfo, moid, BIZ_DESC);
                    } else {
                        if (StringUtils.isNotBlank(resultInfo.getPaymentType()) && resultInfo.getPaymentType().endsWith(BARCODE_NUM)) {
                            ...
                            //拼接地址，将参数拼接在url之后
                            pcAsynScanInfo = returnBarcodeUrlInfo(resultInfo, moid, BIZ_DESC);
                        } else {
                            //使用我司页面
                            pcAsynScanInfo = returnPcPageInfo(resultInfo, moid, BIZ_DESC);
                        }
                    }
                    ...
                    logger.info(BIZ_DESC + moid + "调用TradeCashierPlatHandler处理业务逻辑，处理结果为失败，返回的CommonResult=" + JSONObject.toJSONString(result));
                }
            } else {
                //请求参数不能为空
                result.setRespCode(ErrorCodeEnum.E1017.getCode());
                result.setRespMsg("操作失败");
                logger.info(BIZ_DESC + moid + "接收收银台请求，解析请求参数失败，存在未传的参数，返回的CommonResult=" + JSONObject.toJSONString(result));
            }
        } catch (Exception e) {
            e.printStackTrace();
            result.setRespCode(ErrorCodeEnum.E8001.getCode());
            result.setRespMsg("操作失败");
            logger.error(BIZ_DESC + moid + "接收收银台页面请求异常：" + e.getMessage());
        }
        String resultJson = JSONObject.toJSONString(result);
        logger.info(BIZ_DESC + moid + "处理完业务之后，返回给页面的数据为：" + resultJson);
        return resultJson;
    }
    ...
    public String platMobileCall(HttpServletRequest request, Model model, @PathVariable String mchtId, @PathVariable String mchtOrderId, @PathVariable String paymentType, @PathVariable String extraData) {
        ...
        String mobile = "";
        try {
            moid = "手机端收银台发起支付, 商户号：" + mchtId + "，商户订单号：" + mchtOrderId + ",随机数extraData：" + extraData + "-->支付类型：" + paymentType + "-->";
            String bankCode = request.getParameter("bankCode");
            if (PayTypeEnum.LOCAL_BANK.getCode().equals(bankCode) && StringUtils.isEmpty(bankCode)) {
                logger.info(BIZ_DESC + moid + "银行编码为空");
                result.setRespCode(ErrorCodeEnum.E6110.getCode());
                result.setRespMsg("操作失败");
            ...
            if (StringUtils.isNotBlank(mchtId) && StringUtils.isNotBlank(mchtOrderId) && StringUtils.isNotBlank(paymentType) && StringUtils.isNotBlank(extraData)) {
                ...
                if (null != result && ErrorCodeEnum.SUCCESS.getCode().equals(result.getRespCode()) && null != result.getData()) {
                    ...
                    if (isUseChanCashierPage(result.getData(), moid, BIZ_DESC)) {
                        ...
                        //跳转到上游收银台的中转页面，携带的数据
                        logger.info(BIZ_DESC + moid + "调用TradeCashierPlatHandler处理业务逻辑，处理结果为成功，需要使用上游收银台的中转页面，返回的CommonResult=" + JSONObject.toJSONString(result) + "跳转的页面为：" + page);
                        this.addChanCashierModelInfo(model, result, moid, BIZ_DESC);
                    } else {
                        //非收银台页面跳转
                        page = this.chooseNotCashierPage(deviceType, payType, moid, BIZ_DESC);
                        if (page.endsWith("scan")) {
                            //设置扫码中间页需要的参数
                            this.addScanCentPageModelInfo(model, result, moid, BIZ_DESC);
                            logger.info(BIZ_DESC + moid + "调用TradeCashierPlatHandler处理业务逻辑，处理结果为成功，需要使用中间页，返回的CommonResult=" + JSONObject.toJSONString(result) + "跳转的页面为：" + page);
                            ...
                            logger.info(BIZ_DESC + moid + "调用TradeCashierPlatHandler处理业务逻辑，处理结果为成功，需要使用中间页，但是未找到对应的中间页，返回的CommonResult=" + JSONObject.toJSONString(result) + "跳转的页面为：" + page);
                        }
                    }
                } else {
                    page = this.getPageByDeviceType(deviceType, PageTypeEnum.ERROR.getCode(), moid, BIZ_DESC);
